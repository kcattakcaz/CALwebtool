...
@endif
    <div class="well">
        ...
        @foreach($group->formdefinitions()->get() as $form)
            @if($form->status == 'Reviewing' || $form->status == 'Accepting')
                <h4>{{$form->name}}
        ...
        @if($group->formdefinitions()->where('status','=','Reviewing')->orWhere('status','=','Accepting')->orWhere('status','=','Scored')->get()->count() == 0)
